...
<div class="space-y-8">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
        <div>
            ...
            <p class="text-xs text-slate-400 mt-1">
                ...
                • SMTP: {{ $campaign->smtpSetting ? $campaign->smtpSetting->name : 'Global Default' }} 
                • Stagger Delay: {{ $campaign->delay_seconds }}s
                @if($campaign->resume_path)
                    • 📎 Resume: <span class="font-semibold text-slate-600">{{ basename($campaign->resume_path) }}</span>
                @elseif($campaign->resume_link)
                    • 🔗 Resume Link: <a href="{{ $campaign->resume_link }}" target="_blank" class="text-emerald-600 hover:underline inline-flex items-center gap-0.5 font-semibold">View Link <svg class="w-2.5 h-2.5 inline shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"></path></svg></a>
                @endif
            </p>
        </div>
        ...
        <div class="flex items-center gap-3 shrink-0">
            @if($campaign->status !== 'processing')
                ...
                <form action="{{ route('user.campaigns.send', $campaign->id) }}" method="POST" class="inline">
                    @csrf
                    <button type="submit" class="px-5 py-2 bg-emerald-600 hover:bg-emerald-500 text-white text-sm font-semibold rounded-xl transition-all shadow-md shadow-emerald-600/10 hover:shadow-emerald-600/15">
        ...
        <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm md:col-span-4 flex flex-col justify-between">
            <div>
                <div class="flex items-center justify-between mb-3">
                    <h3 class="text-sm font-bold text-slate-800 uppercase tracking-wider">Application Delivery Progress</h3>
                    <span class="text-lg font-extrabold text-emerald-600" id="campaign-progress-pct">{{ $pct }}%</span>
                </div>
                <div class="w-full bg-slate-100 rounded-full h-3">
                    <div class="bg-emerald-600 h-3 rounded-full transition-all duration-500" id="campaign-progress-bar" style="width: {{ $pct }}%"></div>
                </div>
            </div>

            <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6 pt-6 border-t border-slate-100 text-center">
                <div>
                    <span class="block text-xs font-semibold text-slate-400 uppercase">Total Companies</span>
    ...
    <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-4">
        <h3 class="text-sm font-bold text-slate-800 uppercase tracking-wider border-b border-slate-100 pb-3">Job Application Template</h3>
        <div>
            <span class="block text-xs font-semibold text-slate-400 uppercase">Subject</span>
        ...
        <div>
            <span class="block text-xs font-semibold text-slate-400 uppercase">Email Body</span>
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <div class="p-6 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <div class="flex items-center gap-1.5 self-start">
                <a href="{{ route('user.campaigns.show', ['campaign' => $campaign->id]) }}" 
                   class="px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors border {{ !$statusFilter ? 'bg-emerald-600 text-white border-emerald-600' : 'bg-slate-50 text-slate-600 border-slate-200 hover:bg-slate-100' }}">
                   All
                </a>
                <a href="{{ route('user.campaigns.show', ['campaign' => $campaign->id, 'status' => 'pending']) }}" 
                   class="px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors border {{ $statusFilter === 'pending' ? 'bg-emerald-600 text-white border-emerald-600' : 'bg-slate-50 text-slate-600 border-slate-200 hover:bg-slate-100' }}">
                   Pending
                </a>
                <a href="{{ route('user.campaigns.show', ['campaign' => $campaign->id, 'status' => 'sent']) }}" 
                   class="px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors border {{ $statusFilter === 'sent' ? 'bg-emerald-600 text-white border-emerald-600' : 'bg-slate-50 text-slate-600 border-slate-200 hover:bg-slate-100' }}">
                   Sent
                </a>
                <a href="{{ route('user.campaigns.show', ['campaign' => $campaign->id, 'status' => 'failed']) }}" 
                   class="px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors border {{ $statusFilter === 'failed' ? 'bg-emerald-600 text-white border-emerald-600' : 'bg-slate-50 text-slate-600 border-slate-200 hover:bg-slate-100' }}">
                   Failed
                </a>
            </div>
        </div>
            ...
            <table class="w-full text-left text-sm border-collapse">
                <thead>
                    <tr class="bg-slate-50/50 border-b border-slate-200 text-slate-400 font-medium">
                        <th class="py-4 px-6 text-xs font-semibold uppercase tracking-wider">Recipient (HR)</th>
                        <th class="py-4 px-6 text-xs font-semibold uppercase tracking-wider">Company / Website</th>
                        <th class="py-4 px-6 text-xs font-semibold uppercase tracking-wider">Position</th>
                ...
                <tbody class="divide-y divide-slate-100 text-slate-600" id="recipients-logs-tbody">
                    @forelse($logs as $log)
                        <tr class="hover:bg-slate-50/40 transition-colors">
                            <td class="py-4 px-6">
                                <span class="font-semibold text-slate-900 block">{{ $log->hr_name ?: ($log->name ?: 'No HR Name') }}</span>
                                <span class="text-xs text-slate-400 font-mono block mt-0.5">{{ $log->email }}</span>
                            </td>
                            <td class="py-4 px-6">
                                <span class="font-medium text-slate-800 block">{{ $log->company_name ?: '-' }}</span>
                                @if($log->website)
                                    <a href="{{ $log->website }}" target="_blank" class="text-xs text-emerald-600 hover:underline inline-flex items-center gap-1 font-mono truncate max-w-[150px]" title="{{ $log->website }}">
                                        {{ str_replace(['https://', 'http://', 'www.'], '', $log->website) }}
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="py-8 text-center text-slate-400 text-sm">No email logs found for this filter.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
...
@if($campaign->status === 'processing')
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            const pollProgress = () => {
                fetch(`{{ url('campaigns') }}/${campaignId}/progress`)
                    .then(res => res.json())
                    .then(data => {
