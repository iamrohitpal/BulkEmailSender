...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-slate-900">My Campaigns</h1>
            <p class="text-sm text-slate-500">Track and manage your bulk job application mailings</p>
        </div>
        <a href="{{ route('user.campaigns.create') }}" class="inline-flex items-center gap-2 px-4 py-2 bg-emerald-600 hover:bg-emerald-500 text-white text-sm font-semibold rounded-xl transition-all shadow-md shadow-emerald-600/10">
            ...
            <table class="w-full text-left text-sm border-collapse">
                <thead>
                    <tr class="bg-slate-50/50 border-b border-slate-200 text-slate-400 font-medium">
                        ...
                        <th class="py-4 px-6 text-xs font-semibold uppercase tracking-wider">SMTP Configuration</th>
                ...
                <tbody class="divide-y divide-slate-100 text-slate-600">
                    @forelse($campaigns as $campaign)
                        ...
                        <tr class="hover:bg-slate-50/40 transition-colors">
                            <td class="py-4 px-6">
                                <a href="{{ route('user.campaigns.show', $campaign->id) }}" class="font-semibold text-slate-900 hover:text-emerald-600 transition-colors">
                                    {{ $campaign->name }}
                                </a>
                            ...
                            <td class="py-4 px-6 text-xs text-slate-500 font-medium">
                                {{ $campaign->smtpSetting ? $campaign->smtpSetting->name : 'Global Default SMTP' }}
                            </td>
                            <td class="py-4 px-6">
                            ...
                            <td class="py-4 px-6 text-right space-x-2">
                                <a href="{{ route('user.campaigns.show', $campaign->id) }}" class="inline-flex items-center px-3 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-semibold rounded-lg transition-colors border border-slate-200">
                                    View Details
                                </a>
                                @if($campaign->status !== 'processing')
                                    <a href="{{ route('user.campaigns.edit', $campaign->id) }}" class="inline-flex items-center px-3 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-semibold rounded-lg transition-colors border border-slate-200">
                                        Edit
                                    </a>
                                    <form action="{{ route('user.campaigns.destroy', $campaign->id) }}" method="POST" class="inline" onsubmit="return confirm('Are you sure you want to delete this campaign and all its email records?')">
                                        @csrf
                                        @method('DELETE')
                        ...
                        <tr>
                            <td colspan="5" class="py-12 text-center text-slate-400 text-sm">
                                <div class="flex flex-col items-center justify-center gap-3">
                                    ...
                                    <span>No campaigns created yet. Create a new campaign to begin.</span>
                                </div>
                            </td>
                        </tr>
                    @endforelse
