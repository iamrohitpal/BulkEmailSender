...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div>
        ...
        <p class="text-sm text-slate-500">Modify Campaign: "{{ $campaign->name }}"</p>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-8">
        <form action="{{ route('admin.campaigns.update', $campaign->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="grid grid-cols-1 gap-6">
                ...
                <!-- CSV File Upload -->
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-2">Upload CSV Recipients (Optional)</label>
                    <p class="text-xs text-amber-600 mb-2 font-medium">⚠️ Note: Uploading a new CSV file will completely replace the current {{ $campaign->total_emails }} recipients of this campaign.</p>
                    <div class="flex justify-center px-6 pt-5 pb-6 border-2 border-slate-300 border-dashed rounded-2xl hover:border-indigo-500 transition-colors bg-slate-50/50">
                ...
                <div>
                    ...
                    <textarea name="body" id="body" rows="12" required
                        ...
                        placeholder="Write your mail template here. Support HTML. For example:&#10;&#10;Hi @{{name}},&#10;We wanted to connect with your team at @{{company_name}}...">{{ old('body', $campaign->body) }}</textarea>
                </div>
            </div>

            <!-- Footer Actions -->
            <div class="flex justify-end gap-3 pt-6 border-t border-slate-100">
                <a href="{{ route('admin.campaigns.show', $campaign->id) }}" class="px-5 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 text-sm font-semibold rounded-xl transition-colors border border-slate-200">
                    Cancel
                </a>
